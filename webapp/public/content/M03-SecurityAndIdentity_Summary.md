...
callers.customer -> agents.agent1
callers.employee -> agents.agent1
callers.batch -> agents.agent1
callers.external_agent -> agents.agent1
agents.agent1 -> targets.internal_api
agents.agent2 -> targets.google
agents.agent2 -> targets.servicenow
agents.agent2 -> targets.jira

challenges: "과제 영역" {
  c1: "① 액세스 제어 및 집행"
  c2: "② 권한 부여 및 위임"
  c3: "③ 자격 증명 수명 주기 관리 및 규모 조정"
  c4: "④ SaaS 및 서드 파티 통합"
  c5: "⑤ 거버넌스 및 규정 준수"
}
```
...
user: "사용자"
app: "앱"
inbound_auth: "인바운드 인증"
agent: "에이전트"

user -> app -> inbound_auth -> agent
inbound_auth: {
  q1: '"이 사용자는 누구인가?"' { shape: text }
  q2: '"이 에이전트를 호출할 수 있는가?"' { shape: text }
}
```
...
caller: "호출자 (IAM 사용자 또는 역할)"
http_request: "HTTP 요청 + Authorization 헤더\nAWS4-HMAC-SHA256\nCredential + SignedHeaders + Signature"
service: "AWS 서비스 엔드포인트\n(예: S3, DynamoDB, Bedrock 등)"
result: "요청 승인 또는 거부 (403 Forbidden)"

caller -> http_request: "1. Secret Access Key로 서명 생성\n2. Access Key ID + 서명을 헤더에 포함"
http_request -> service
service -> result: "3. Access Key ID로 Secret Key 내부 조회\n4. 동일 알고리즘으로 서명 재계산\n5. 서명 일치 → 신원 확인 완료\n6. IAM 정책 평가"
```
...
```
[개념 레벨 - 5가지 인증 방식]       [Runtime 구현 레벨 - 2가지 구성]
──────────────────────────────    ──────────────────────────────────
Amazon Cognito      ─────┐
페더레이션(SSO)      ─────┼──→    JWT Bearer 토큰 인증
JWT Bearer 토큰     ─────┘        (Custom JWT Authorizer)
IAM SigV4           ──────────→   IAM SigV4 인증
API 키              ──────────→   (Runtime 외부에서 처리 - API Gateway 등)
```
...
```d2
# OAuth 2-Legged (2LO) — 클라이언트 자격 증명 흐름

agent: "에이전트"
auth_server: "인증 서버\n(토큰 엔드포인트)"
external_service: "외부 서비스\n(API)"

agent -> auth_server: "① POST /token (grant_type=client_credentials)\n+ Client ID + Client Secret\n+ scope (요청 범위)"
auth_server -> agent: "② 200 OK { access_token, token_type, expires_in }\n(사용자 개입 없이 즉시 발급)"
agent -> external_service: "③ API 호출 + Authorization: Bearer <access_token>\n(에이전트 자체 권한으로 리소스 접근)"
```

```python
# AgentCore Identity SDK - 2LO (Client Credentials) 사용 예
...
@requires_access_token(
    provider_name="my-service-provider",      # 사전 등록된 자격 증명 공급자 이름
    scopes=["api.read", "api.write"],         # 요청할 OAuth 스코프
    auth_flow="CLIENT_CREDENTIALS"            # 2LO: 사용자 개입 없음
)
async def call_external_api(*, access_token: str):
    """에이전트 자체 권한으로 외부 서비스 호출"""
    import httpx
...
agent -> user: "① 에이전트가 인증 URL 생성"
user -> auth_server: "② 사용자가 브라우저에서 로그인 + 동의"
auth_server -> user: "③ 인증 코드를 콜백 URL로 전달"
user -> agent: "③ 인증 코드 전달"
agent -> auth_server: "④ 인증 코드 → 토큰 교환"
auth_server -> agent: "⑤ Access Token + Refresh Token"
agent -> agent: "⑥ 토큰 볼트에 저장 (사용자별)"
agent -> external_service: "⑦ 사용자 대신 API 호출"
external_service -> agent: "⑧ 응답"
```
...
```python
# AgentCore Identity SDK - 3LO (Authorization Code Grant) 사용 예
# 출처: AgentCore Starter Toolkit Quickstart
...
async def handle_auth_url(url):
    """인증 URL을 사용자에게 전달 (채팅 UI, 이메일 등으로 전송)"""
    # 사용자가 이 URL을 브라우저에서 열어 로그인 + 동의
    print(f"사용자 인증 필요 - 브라우저에서 열기: {url}")

@requires_access_token(
    provider_name="google-drive-provider",    # 사전 등록된 자격 증명 공급자
    scopes=["https://www.googleapis.com/auth/drive.readonly"],
    auth_flow="USER_FEDERATION",              # 3LO: 사용자 동의 필요
    on_auth_url=handle_auth_url,              # 인증 URL 콜백 (사용자에게 전달)
    force_authentication=False                # True면 매번 재인증 강제
)
async def list_user_drive_files(*, access_token: str):
    """사용자 대신 Google Drive 파일 목록 조회"""
    import httpx
    async with httpx.AsyncClient() as client:
        response = await client.get(
...
user: "사용자"
idp: "Amazon Cognito / Okta / Entra ID\n(사용자 인증 + JWT 발급)"
apigw: "Amazon API Gateway\n(Cognito Authorizer 또는 Lambda Authorizer)\n(JWT 검증, 스코프 확인)"
agent: "에이전트 — ECS/EKS/Lambda"
...
# OAuth 토큰 방식
agent1: "에이전트"
secrets: "AWS Secrets Manager\n(OAuth 클라이언트 시크릿, API 키 저장)"
lambda_rotate: "Lambda 순환 함수"
token_endpoint: "토큰 엔드포인트"
external: "외부 서비스"

agent1 -> secrets
secrets -> lambda_rotate
lambda_rotate -> token_endpoint: "OAuth 토큰 갱신 자동화"
agent1 -> external: "액세스 토큰"

# AWS 서비스 접근 방식
agent2: "에이전트 (AWS 접근)"
iam_sts: "IAM Role + STS AssumeRole\n(임시 자격 증명 발급)"
aws_service: "AWS 서비스"

agent2 -> iam_sts
agent2 -> aws_service: "임시 자격 증명"
```
...
identity: "AgentCore Identity" {
  workload_cred: "워크로드 자격 증명"
  token_store: "토큰 저장소"
  token_cache: "토큰 캐싱"
  cred_provider: "자격 증명 공급자"
}

aws_resources: "AWS 리소스\n(IAM)"
gateway: "AgentCore\nGateway"
external: "외부 리소스\n(OAuth/API키)"

user -> app -> inbound_auth -> runtime
runtime -> identity
identity -> aws_resources
identity -> gateway
identity -> external
```
...
aws_cloud: "AWS Cloud" {
  ecr: "① Amazon ECR\n컨테이너 이미지 생성\n(auto_create_ecr)\n← agent.py + requirements.txt 패키징"

  runtime: "② AgentCore Runtime" {
    agent_info: "에이전트: customer_support_agent\n실행 역할: execution_role_arn" { shape: text }
    jwt_auth: "JWT Authorizer 구성" {
      config_detail: "allowedClients: [cognito_client_id]\ndiscoveryUrl: cognito_discovery_url" { shape: text }
    }
  }

  cognito: "③ Amazon Cognito (또는 외부 IdP)\nJWKS 엔드포인트\n(공개 키 제공 — 서명 검증용)\n← discoveryUrl로 자동 발견"
}

developer -> aws_cloud.ecr: ".launch()"
aws_cloud.ecr -> aws_cloud.runtime
aws_cloud.runtime -> aws_cloud.cognito: "토큰 검증 시"
```
...
```d2
# AgentCore Runtime 실행 역할 (IAM Role)

execution_role: "AgentCore Runtime 실행 역할 (IAM Role)" {
  basic: "기본 권한" {
    p1: "AgentCore Identity: 워크로드 액세스 토큰 가져오기"
    p2: "AWS X-Ray: 샘플링 규칙, 추적 넣기"
    p3: "CloudWatch: 로그 그룹/스트림/이벤트 생성"
    p4: "Amazon ECR: 이미지/계층 가져오기, 인증 토큰"
    p5: "모든 모델: 스트리밍 유무에 관계없이 모델 간접 호출"
  }

  additional: "추가 가능" {
    p6: "다른 AWS 서비스 작업 (S3, DynamoDB 등)"
    p7: "다른 AgentCore 서비스 (Memory, Gateway 등)"
  }
}
```
...
```
사용자 ──[최초 인증]──→ 에이전트
에이전트 ──[OBO Token Exchange]──→ 서비스 A (사용자 대신)
에이전트 ──[OBO Token Exchange]──→ 서비스 B (사용자 대신)
에이전트 ──[OBO Token Exchange]──→ 서비스 C (사용자 대신)
```
- 다중 동의 흐름 불필요
