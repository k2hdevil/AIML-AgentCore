...
start: "도구가 필요한가?"
q_managed: "AWS 관리형 도구로 충분한가?"
q_existing: "기존 API/서비스가 있는가?"
q_custom: "커스텀 로직이 필요한가?"

browser: "AgentCore Browser" { shape: hexagon }
code_interpreter: "AgentCore Code Interpreter" { shape: hexagon }
gw_openapi: "Gateway (OpenAPI 대상)" { shape: hexagon }
gw_lambda: "Gateway (Lambda 대상)" { shape: hexagon }
gw_mcp: "Gateway (MCP 서버 대상)" { shape: hexagon }
local_mcp: "로컬 MCP 서버 (Runtime 내)" { shape: hexagon }
remote_mcp: "원격 MCP 서버" { shape: hexagon }

start -> q_managed
start -> q_existing
start -> q_custom

q_managed -> browser: 웹 탐색 필요
q_managed -> code_interpreter: 코드 실행 필요

q_existing -> gw_openapi: REST API
q_existing -> gw_lambda: Lambda 함수
q_existing -> gw_mcp: MCP 서버

q_custom -> local_mcp: 지연 시간 중요
q_custom -> remote_mcp: 공유/독립 스케일링
```
...
user_query: '사용자 쿼리 ("Amazon에서 신발 구매")'
agent: "에이전트"
llm: "LLM (지침을 명령으로 변환)"
tool_call: '컴퓨터 도구 호출:\n{type: "click", button: "left", x:286, y:102}'
browser_env: "실행 환경 (헤드리스 브라우저)"
result: "명령 실행 → 스크린샷 캡처 → 결과 반환"

user_query -> agent
agent -> llm: "LLM 간접 호출"
llm -> tool_call
agent -> browser_env
browser_env -> result -> agent
```
...
cdp_conn: "CDP WebSocket 연결" { shape: text }

browser_infra: "AWS 관리형 — AgentCore Browser" {
  microvm: "격리된 MicroVM (Firecracker)" {
    chrome: "헤드리스 Chrome 브라우저" {
      features: "• Chrome DevTools Protocol (CDP) 엔드포인트 노출\n• Chrome Enterprise Policies (100+ 정책)\n• 브라우저 확장, 프로필, 프록시 구성\n• Custom Root CA (내부 서비스 SSL)\n• OS-Level Interaction (마우스, 키보드, 시스템 알림)" { shape: text }
    }
    session_mgmt: "• 세션 관리 (생성/일시정지/종료)\n• DCV 라이브 뷰 스트리밍\n• Web Bot Auth (CAPTCHA 감소)\n• VPC 연결 (내부 웹 앱 접근)" { shape: text }
  }
  managed: "관리 항목: 프로비저닝, 스케일링, 격리, 보안 패치, 관찰성" { shape: text }
}

user_code -> cdp_conn
cdp_conn -> browser_infra
```
...
**작동 흐름**:
```d2
# Code Interpreter 작동 흐름

user_query: "사용자 쿼리"
agent: "에이전트"
llm: "LLM (도구 선택)"
session: "Code Interpreter 세션 생성" {
  shell: "쉘 (코드 실행)"
  fs: "파일 시스템 (데이터 저장)"
  obs: "Observability (원격 측정)"
}
result: "도구 결과 반환 → 에이전트"
...
host_process: "애플리케이션 호스트 프로세스" {
  host: "호스트 (앱, LLM, IDE 등)" {
    client1: "MCP 클라이언트 1"
    client2: "MCP 클라이언트 2"
  }
}
...
host_process.host.client1 -> server1
host_process.host.client2 -> server2
server1 -> resource_a
server2 -> resource_b
```
...
api_target: "API 엔드포인트 대상" {
  desc: "OpenAPI 스키마\nRESTful 서비스"
}
lambda_target: "AWS Lambda 대상" {
  desc: "Lambda 함수\n도구1, 2, 3"
}
mcp_target: "MCP 서버 대상" {
  desc: "MCP 서버\n도구1, 2, 3"
}

features: "기능: 도구 나열 | 도구 간접 호출 | 시맨틱 검색" { shape: text }

agent -> mcp_client -> gateway
gateway -> api_target
...
# 도구 수가 적을 때
small: "도구 수가 적을 때 (≤30개)" {
  list: "tools/list (전체 나열)"
  llm_select: "LLM이 전체 목록에서 선택"
  list -> llm_select
}

# 도구 수가 많을 때
large: "도구 수가 많을 때 (300개+)" {
  search: "시맨틱 검색 (자연어 의도)"
  filter: "관련 도구 10개만 필터링"
  llm_pick: "LLM이 10개 중 선택"
  search -> filter -> llm_pick
}
```
...
tool_schema: "도구 스키마" {
  desc: "이벤트 객체는 도구\n스키마와 일치해야 함" { shape: text }
}

lambda_event: "Lambda 이벤트 객체"

lambda_context: "Lambda 컨텍스트 사용자 지정 객체 (Gateway 메타)" {
  meta: "• bedrockAgentCoreMessageVersion\n• bedrockAgentCoreAwsRequestId\n• bedrockAgentCoreMcpMessageId\n• bedrockAgentCoreGatewayId\n• bedrockAgentCoreTargetId\n• bedrockAgentCoreToolName" { shape: text }
}

mcp_endpoint -> gateway -> iam_role -> lambda
gateway -> tool_schema
lambda -> lambda_event
...
openapi_target: "OpenAPI 대상" {
  rest_api: "REST API 서비스\n(OpenAPI 스키마)"
}

agent -> inbound_auth: "/mcp"
inbound_auth -> gateway
gateway -> outbound
outbound.identity -> mcp_target: "토큰"
outbound.cred_provider -> openapi_target: "API키/토큰"
```
...
agent_mcp: "에이전트/MCP 클라이언트"
policy: "AgentCore Policy" {
  features: "• 정책 수명 주기\n• NL2Cedar 변환\n• 동적 정책 평가\n• 허용/거부 결정" { shape: text }
}
policy_admin: "정책 관리자 (정책 작성/관리)"

allow_path: "허용 → Gateway → 도구/API/시스템"
deny_path: "거부 → Observability (감사 로그)"

agent_mcp -> policy: "인바운드 도구 호출"
policy_admin -> policy
policy -> allow_path
policy -> deny_path
```
...
registry: "AWS Agent Registry" {
  ...
  access: "접근 방식" {
    a1: "Console UI"
    a2: "API"
    a3: "MCP 서버 (IDE에서 직접 쿼리)"
  }

  security: "보안: IAM + OAuth (Custom JWT)" { shape: text }
}
```
