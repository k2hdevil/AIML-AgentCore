...
agentcore_memory: "AgentCore Memory" {
  ...
  agent_impl: "에이전트 구현" {
    save: "메시지 저장 (동기식)"
    search_event: "이벤트 검색 (동기식)"
    search_record: "메모리 레코드 검색 (비동기식)"
  }
}
```
...
consolidation: "통합 (ADD/UPDATE/SKIP)\n비슷한 메모리 검색"

short_term -> extraction
extraction -> long_term
long_term -> consolidation
```
...
short_term_store: "단기 메모리 (원시 이벤트 저장)"

extraction: "추출 (Extraction)\nLLM이 원시 대화에서\n인사이트 식별·추출"
consolidation: "통합 (Consolidation)\n기존 메모리와 비교·판단\nADD/UPDATE/SKIP"
embedding: "벡터 임베딩 + 인덱싱\n(장기 메모리 레코드 저장)"
retrieval: "RetrieveMemoryRecords()로 시맨틱 검색 가능"

user_dialog -> create_event -> short_term_store
short_term_store -> extraction: "비동기 트리거"
