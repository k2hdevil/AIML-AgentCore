...
```d2
# Managed Harness 아키텍처

title: "Managed Harness 아키텍처" { shape: text; style.font-size: 20 }

developer: "개발자 선언 (harness.json 또는 API)" {
  config: |md
...
managed: "AgentCore 자동 관리 (격리된 microVM)" {
  loop: "에이전틱 루프: 추론 → 도구 선택 → 실행 → 응답 스트리밍"
  isolation: "세션 격리: 각 세션 독립 microVM"
  obs: "Observability: 모델/도구/메모리/셸 모든 스팬 자동 기록"
  cost: "비용 제어: 토큰 예산, 반복 제한, 타임아웃"
  fs: "파일시스템: 영구 세션 스토리지 + S3/EFS 마운트"
}
...
│                                                                 │
│  ┌──────────┐  ┌──────────────────────────────────────────┐     │
│  │  Chat     │  │  Timeline (트레이스 시각화)                │     │
│  │           │  │                                          │     │
│  │  User:    │  │  ├─ LLM Call (1.2s)                      │     │
│  │  "주문    │  │  │   └─ Input: 450 tokens                │     │
│  │   상태"   │  │  ├─ Tool: get_order (0.3s)               │     │
│  │           │  │  │   └─ orderId: "12345"                 │     │
│  │  Agent:   │  │  ├─ LLM Call (0.8s)                      │     │
│  │  "주문    │  │  │   └─ Output: 120 tokens               │     │
...
```d2
# AgentCore Payments 전체 흐름

try_call: "에이전트가 유료 API 호출 시도"
http402: "HTTP 402 Payment Required 응답 수신"

payments: "AgentCore Payments" {
  auth: "1. 지갑 인증 (Wallet Authentication)\n• Coinbase: 스테이블코인 (USDC)\n• Stripe/Privy: 법정화폐 (USD)"
  governance: "2. 정책 확인 (Spending Governance)\n• 예산 한도 확인\n• 승인 정책 평가\n• 거래 금액 검증"
  transaction: "3. 거래 실행 (Transaction)\n• 마이크로페이먼트 (< \$0.01/호출)\n• 실시간 결제 완료"
  observability: "4. 관찰성 (Observability)\n• 거래 이력 CloudWatch 기록\n• 비용 분석 대시보드"

  auth -> governance -> transaction -> observability
}

success: "유료 API 정상 응답 수신 → 에이전트 작업 계속"
...
agent -> paid_api: "1. GET /premium-data"
paid_api -> agent: "2. HTTP 402 + 결제 요구 (금액/지갑 주소)"
agent -> payments: "3. 결제 요청"
payments -> coinbase: "4. 스테이블코인 전송"
payments -> agent: "5. 결제 증명 (receipt)"
agent -> paid_api: "6. GET /premium-data + 결제 증명"
paid_api -> agent: "7. 200 OK + 데이터"
```
...
registry: "AWS Agent Registry" {
  ...
  security: "보안\n• IAM 정책 기반 접근 제어\n• OAuth (Custom JWT) 인증\n• 리소스별 가시성 제어 (public/private/team)"
}
```
...
observe: "1. 관찰 (Observability)\n• 트레이스\n• 메트릭\n• 로그"
evaluate: "2. 평가 (Evaluations)\n• 온라인\n• 배치\n• 시뮬레이션"
optimize: "3. 최적화 (Optimization)\n• 분석\n• 추천\n• A/B 테스트"
deploy: "4. 배포 (Runtime)\n• 새 버전\n• A/B\n• 롤아웃"

observe -> evaluate -> optimize -> deploy
deploy -> observe: "지속적 반복"
```
...
collect: "프로덕션 트레이스 수집 (Observability)"
analyze: "평가 결과 분석\n• 도구 선택 정확도가 낮은 패턴 식별\n• 목표 성공률이 떨어지는 시나리오 발견\n• 유해성 점수가 높은 응답 패턴 감지"
recommend: "개선 추천 생성\n• 시스템 프롬프트 보강\n• 도구 설명 개선\n• 응답 길이 지침 추가"
abtest: "A/B 테스트 실행\n• Control (기존 버전): 50% 트래픽\n• Treatment (개선 버전): 50% 트래픽\n• 통계적 유의성 확인 (p < 0.05)"
result: "결과에 따라\n• 개선 확인 → 전체 롤아웃\n• 차이 없음/악화 → 롤백"

collect -> analyze -> recommend -> abtest -> result
```
...
pipeline: "CI 파이프라인" {
  build: "빌드 & 단위 테스트"
  batch: "Batch Evaluation 실행\n• 테스트 세션 리플레이 (10-50개)\n• 이전 vs 새 버전 점수 비교\n• 회귀 감지 시 → PR 블록 + 알림"
  simulation: "User Simulation (선택)\n• 가상 사용자 5-10명 × 10턴 대화"
  gate: "품질 게이트\n• 모든 지표 ≥ 임곗값 → 배포 승인"

  build -> batch -> simulation -> gate
}
...
m01: "M01 기초"
m02: "M02 Runtime"
h_cli: "🆕 Harness + CLI 소개 (15-20분)"
m03: "M03 보안"
m04: "M04 도구/Gateway"
pay_reg: "🆕 Payments + Registry 소개 (15-20분)"
m05: "M05 메모리"
m06: "M06 관찰성/평가"
perf: "🆕 Performance Loop 소개 (15-20분)"
m07: "M07 마무리"
