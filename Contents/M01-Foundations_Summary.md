...
```d2
# 기존 생성형 AI vs 에이전틱 AI 아키텍처 비교

traditional: "기존 생성형 AI 애플리케이션" {
  flow: "사용자 → 애플리케이션 → LLM → 응답\n(단일 턴 대화)"
}

agentic: "에이전틱 AI 애플리케이션" {
  user: 사용자
  app: 애플리케이션
  agent_loop: "에이전틱 루프" {
    reasoning: 추론
...
agent_arch: "에이전트 아키텍처" {
  framework: "1. 프레임워크 또는 에이전트 코드\n(오케스트레이션 레이어)"
...
act -> observe
observe -> goal_check
goal_check -> final_response: "Yes"
goal_check -> perceive: "No (루프 계속)"
```
...
agent: "고객 서비스 에이전트" {
  framework: "프레임워크: Strands Agents"
  model: "모델: Amazon Bedrock (모든 모델)"

  tools: "도구 (Tools via Gateway)" {
    t1: "get_product_info() — 제품 정보 조회"
    t2: "get_return_policy() — 반품 정책 조회"
    t3: "get_technical_support() — 기술 지원 정보"
    t4: "Web Browser — 웹 탐색"
    t5: "🆕 Elicitation — 사용자 확인 요청"
  }

  short_term: "단기 메모리\n(세션 컨텍스트)"
  long_term: "장기 메모리\n(고객 선호도)\n🆕 메타데이터 필터"

  infra: "인프라: Runtime | Identity | Observability | Gateway\nPolicy | Evaluations | MCP\n🆕 Payments | Registry | CLI" {
    style.stroke-dash: 3
  }
}
```
...
agent_a -> agent_b -> agent_c -> result
```
